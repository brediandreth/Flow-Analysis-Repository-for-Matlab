function out = fit_persist1(samps, channelIDs, my_morpho_gate, AF, comp_matrix, bin_order, bin_eval)
    % fit with 1 independent variable (dimensions) with 2 datasets: a
    % positive control and a degraded sample 
...
%     F_enz = @(a,D) lin2logicle((sqrt((-a(1).*logicle2lin(D) + a(2) + a(4).*a(3)).^2 + 4.*a(1).*logicle2lin(D).*a(4).*a(3)) + a(1).*logicle2lin(D) - a(2) - a(4).*a(3))./(2.*a(3)));
   ...
   funcs2 =  {F_lin,F_enz};
    ...
    for i = 1:length(samps)
            ...
            xdat = dat_comp(:,bin_order); % inputs
